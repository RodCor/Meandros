...
class Root(Tk):
    ...
    def roi_detection(self):
        ...
        if not self.object_list[index].ctrlPoints:
            approx_roi, class_id = roi_detect.model_worker(image_tif)
        ...
        if self.object_list[index].roi is not None and self.object_list[index].ctrlPoints is not None:
            ...
            print("ROI Saved")
            # import csv
            # f = open('Contour_BrightField.csv', 'a+')
            # writer = csv.writer(f)
    ...
    def amputation_plane(self):
        ...
        if filename[-3:] == "tif":
            image_tif = fix_tif(filename)
        ap_plane, intersect_ap_list = bezierPD.BezierPD(filename=image_tif, roi=self.object_list[index].roi).run("Amputation Plane")
