...
def analysis(p_d, conj_mask, ap, image, channel, threshold, landmarks):
    ...
    for k in range(len(p_d)):
        ...
        #intersection lineas de lectura & roi -> R
        ...
        area_corr = math_aux.f_corr(angulo, area) #area corregida pro el factor de corrección

        #intersection = lineas de lectura
        if intersection & ap == set(): #Izquierda del plano de amputacion
            suma = 0
            for i in range(len(intersection_list)):
                ...
                if px[channel] > threshold:  # THRESHOLD
                    suma += 1 #only pos
                    img1[intersection_list[i][1], intersection_list[i][0]] = [0, 0, 255] #pinta la señal del thresh
            rango += 1
            areas.append(area) #rm
            ...
            if intersection & elbow_landmark != set():
                ...
                wrist.append(rango)
        elif intersection & ap != set(): #Region colapsada que cruza con plano amputacion
            region.append(k)

    #print(region, rango, len(p_d))

    maximo_f = max(pixels) #Maximo de las lineas de lectura
    ...
    # Divided by the size of the reading lines (3)
    # 5.16 UM=PIXEL
